...
This skill adds Slack support to AquaClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.aquaclaw/state.yaml`. If `slack` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
```bash
pnpm dlx tsx scripts/apply-skill.ts .claude/skills/add-slack
```
...
>
> The JID format for AquaClaw is: `slack:C0123456789`
...
4. Service is running: `launchctl list | grep aquaclaw`
...
6. Restart: `launchctl kickstart -k gui/$(id -u)/com.aquaclaw`
