# Slack App Setup for AquaClaw

Step-by-step guide to creating and configuring a Slack app for use with AquaClaw.

## Prerequisites

- A Slack workspace where you have admin permissions (or permission to install apps)
- Your AquaClaw instance with the `/add-slack` skill applied
...
3. When prompted for a token name, enter something like `aquaclaw`
4. Click **Generate**
...
You need the Slack channel ID to register it with AquaClaw.
...
The AquaClaw JID format is `slack:` followed by the channel ID, e.g., `slack:C0123456789`.
