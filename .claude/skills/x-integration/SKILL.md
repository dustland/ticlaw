---
name: x-integration
description: X (Twitter) integration for AquaClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** AquaClaw v1.0.0. Directory structure may change in future versions.
...
1. **AquaClaw is installed and running** - WhatsApp connected, service active
...
npx dotenv -e .env -- pnpm dlx tsx .claude/skills/x-integration/scripts/setup.ts
...
# Verify: launchctl list | grep aquaclaw (macOS) or systemctl --user status aquaclaw (Linux)
```
...
| `AQUACLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/aquaclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into AquaClaw, make the following modifications:
...
All paths below are relative to project root (`AQUACLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep aquaclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status aquaclaw
```
...
```bash
echo '{"content":"Test tweet - please ignore"}' | npx dotenv -e .env -- pnpm dlx tsx .claude/skills/x-integration/scripts/post.ts
```

### Test Like

```bash
echo '{"tweetUrl":"https://x.com/user/status/123"}' | npx dotenv -e .env -- pnpm dlx tsx .claude/skills/x-integration/scripts/like.ts
```
...
echo '{"content":"Test"}' | pnpm dlx tsx .claude/skills/x-integration/scripts/post.ts
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/aquaclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/aquaclaw.log | tail -20
```
...
docker run aquaclaw-agent ls -la /app/src/skills/
```
