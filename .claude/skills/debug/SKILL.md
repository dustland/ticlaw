...
|-----|----------|---------|
| **Main app logs** | `logs/aquaclaw.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/aquaclaw.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash aquaclaw-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash aquaclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash aquaclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash aquaclaw-agent:latest
```
...
# Also clear the session ID from AquaClaw's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/aquaclaw.log | tail -5
...
```bash
echo "=== Checking AquaClaw Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo aquaclaw-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/aquaclaw.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
