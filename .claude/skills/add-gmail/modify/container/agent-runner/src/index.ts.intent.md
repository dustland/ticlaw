...
- Added: `gmail` MCP server alongside the existing `aquaclaw` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `aquaclaw` MCP server configuration is unchanged
...
## Must-keep
- The `aquaclaw` MCP server with its environment variables
