---
name: add-gmail
description: Add Gmail integration to AquaClaw. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail support to AquaClaw — either as a tool (read, send, search, draft) or as a full channel that polls the inbox.

## Phase 1: Pre-flight

### Check if already applied

Read `.aquaclaw/state.yaml`. If `gmail` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
Add `gmail` to `.aquaclaw/state.yaml` under `applied_skills` with `mode: tool-only`.
...
```bash
pnpm dlx tsx scripts/apply-skill.ts .claude/skills/add-gmail
```
...
- Records the application in `.aquaclaw/state.yaml`
...
>    - Application type: **Desktop app**, name: anything (e.g., "AquaClaw Gmail")
...
Tell the user to send themselves a test email. The agent should pick it up within a minute. Monitor: `tail -f logs/aquaclaw.log | grep -iE "(gmail|email)"`.
...
3. Remove `gmail` from `.aquaclaw/state.yaml`
4. Clear stale agent-runner copies: `rm -r data/sessions/*/agent-runner-src 2>/dev/null || true`
5. Rebuild: `cd container && ./build.sh && cd .. && pnpm run build && launchctl kickstart -k gui/$(id -u)/com.aquaclaw` (macOS) or `systemctl --user restart aquaclaw` (Linux)
...
6. Remove `gmail` from `.aquaclaw/state.yaml`
7. Clear stale agent-runner copies: `rm -r data/sessions/*/agent-runner-src 2>/dev/null || true`
8. Rebuild: `cd container && ./build.sh && cd .. && pnpm run build && launchctl kickstart -k gui/$(id -u)/com.aquaclaw` (macOS) or `systemctl --user restart aquaclaw` (Linux)
