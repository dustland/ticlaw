...
This skill adds Telegram support to AquaClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.aquaclaw/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep aquaclaw` (macOS) or `systemctl --user status aquaclaw` (Linux)
...
- Check bot is started: `tail -f logs/aquaclaw.log`
