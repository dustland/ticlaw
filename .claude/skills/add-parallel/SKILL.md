...
Adds Parallel AI MCP integration to AquaClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses AquaClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. AquaClaw already set up and running
...
Add Parallel AI MCP servers after the aquaclaw server:
```typescript
const mcpServers: Record<string, any> = {
  aquaclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__aquaclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__aquaclaw__send_message
   4. Use mcp__aquaclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo aquaclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep aquaclaw  # macOS
...
- Check task runs: `tail -f logs/aquaclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.aquaclaw` (macOS) or `systemctl --user restart aquaclaw` (Linux)
