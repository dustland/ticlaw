...
# Expected: PID  0  com.aquaclaw (PID = running, "-" = not running, non-zero exit = crashed)

# 2. Any running containers?
container ls --format '{{.Names}} {{.Status}}' 2>/dev/null | grep aquaclaw

# 3. Any stopped/orphaned containers?
container ls -a --format '{{.Names}} {{.Status}}' 2>/dev/null | grep aquaclaw

# 4. Recent errors in service log?
grep -E 'ERROR|WARN' logs/aquaclaw.log | tail -20

# 5. Is WhatsApp connected? (look for last connection event)
grep -E 'Connected to WhatsApp|Connection closed|connection.*close' logs/aquaclaw.log | tail -5

# 6. Are groups loaded?
grep 'groupCount' logs/aquaclaw.log | tail -3
```
...
grep -E 'Container timeout|timed out' logs/aquaclaw.log | tail -10
...
grep -E 'Scheduling retry|retry|Max retries' logs/aquaclaw.log | tail -10
```
...
grep 'New messages' logs/aquaclaw.log | tail -10

# Check if messages are being processed (container spawned)
grep -E 'Processing messages|Spawning container' logs/aquaclaw.log | tail -10

# Check if messages are being piped to active container
grep -E 'Piped messages|sendMessage' logs/aquaclaw.log | tail -10

# Check the queue state — any active containers?
grep -E 'Starting container|Container active|concurrency limit' logs/aquaclaw.log | tail -10
...
grep -E 'Mount validated|Mount.*REJECTED|mount' logs/aquaclaw.log | tail -10

# Verify the mount allowlist is readable
cat ~/.config/aquaclaw/mount-allowlist.json
...
container run -i --rm --entrypoint ls aquaclaw-agent:latest /workspace/extra/
```
...
grep 'QR\|authentication required\|qr' logs/aquaclaw.log | tail -5
