...
<dict>
    ...
    <string>${projectRoot}/logs/aquaclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/aquaclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.aquaclaw');
  } catch {
...
/**
 * Kill any orphaned aquaclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned aquaclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/aquaclaw.service';
    ...
    unitPath = path.join(unitDir, 'aquaclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=AquaClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/aquaclaw.log
StandardError=append:${projectRoot}/logs/aquaclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned aquaclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable aquaclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start aquaclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active aquaclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-aquaclaw.sh');
  const pidFile = path.join(projectRoot, 'aquaclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-aquaclaw.sh — Start AquaClaw without systemd',
    ...
    '    echo "Stopping existing AquaClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting AquaClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/aquaclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/aquaclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "AquaClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/aquaclaw.log"`,
  ];
